...
- **Every full-screen `fixed inset-0` overlay/sheet/modal rendered from within a routed page MUST be wrapped in `<Portal>` (`frontend/src/components/portal.tsx` → `createPortal(children, document.body)`).** The app wraps every routed page in `.anim-page-in`, whose CSS `transform` PERSISTS (animation fill mode `both`). A non-`none` `transform` on an ancestor **re-bases `position: fixed` to that ancestor instead of the viewport**, so an inline `fixed inset-0` gets sized to the tall, scrolled page and clipped by `PullToRefresh`'s `overflow-y-auto` — the overlay renders **blank/off-screen** (the `shrink-0` header shows, the `flex-1` body vanishes; `max-h-[N%]` sheets also mis-size). Symptom we hit repeatedly: the English «Закрепить»/«Наговори» drill opened to a black empty screen with only the header. jsdom (RTL) and an ancestor-less HTML harness both render fine — **reproduce fixed-overlay layout bugs in a real browser with the actual ancestor chain (Playwright + `getBoundingClientRect`)**, and check `overlay.height === viewport.height`. Exceptions that already portal correctly: vaul `Drawer.*` (portals internally) and overlays rendered at AppShell level (e.g. `VoiceOverlay`), which sit outside the `.anim-page-in` wrapper. When adding a new overlay, grep-guard: a `className="…fixed inset-0…"` in a file that doesn't import `Portal` is a bug.
